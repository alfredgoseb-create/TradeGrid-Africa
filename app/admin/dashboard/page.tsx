...
export default function Dashboard() {
  ...
  async function handleUpdate(product) {
    ...
    if (error) alert("Failed to update: " + error.message);
      ...
      // Send low stock alert email if stock <= 10 and > 0
      const newStock = Number(product.stock_level);
  ...
  const lowStockProducts = products.filter(p => Number(p.stock_level) <= 10 && Number(p.stock_level) > 0);
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="bg-white rounded-xl shadow p-6 mb-8">
          ...
          <div className="mt-4 p-3 bg-yellow-50 rounded-lg">
            <h3 className="font-medium text-yellow-800">⚠️ Low stock (≤10 units)</h3>
            {lowStockProducts.length === 0 ? (
              <p className="text-sm text-yellow-600">No low stock products</p>
            ) : (
              <ul className="text-sm space-y-1 text-yellow-700">
                {lowStockProducts.map(p => (
                  <li key={p.id}>• {p.name} – {p.stock_level} left</li>
                ))}
              </ul>
            )}
          </div>
        ...
        <div className="bg-white rounded-xl shadow p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={fetchProducts} className="text-sm px-3 py-1 bg-gray-100 rounded-lg">
              Refresh
            </button>
          </div>
          {loading ? (
                ...
                <div key={product.id} className="border rounded-xl p-4 flex gap-4 items-start">
                  ...
                  {product.image_url ? (
                    <img
                      ...
                      alt={product.name}
                      className="h-24 w-24 object-cover rounded-lg flex-shrink-0 shadow-sm"
                    />
                  ) : (
                    <div className="h-24 w-24 bg-gray-100 rounded-lg flex-shrink-0 flex items-center justify-center text-gray-400 text-xs">
                      No image
                    </div>
                  )}
                      ...
                      <div className="space-y-2">
                        ...
                        <div>
                          ...
                          {selectedProduct.image_url && (
                            <img src={selectedProduct.image_url} alt="Preview" className="mt-2 h-16 w-16 object-cover rounded" />
                          )}
                        </div>
                        <div className="flex gap-2">
                          <button className="px-4 py-2 rounded bg-green-600 text-white" onClick={() => handleUpdate(selectedProduct)}>
                            Save
                          </button>
                          <button className="px-4 py-2 rounded bg-gray-100" onClick={() => setSelectedProduct(null)}>
                            Cancel
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                        ...
                        <div className="flex flex-col gap-1 md:items-end">
                          <div className="flex items-center gap-2">
                            ...
                            {Number(product.stock_level) <= 10 && Number(product.stock_level) > 0 && (
                          ...
                          <div className="flex gap-2 mt-1">
                            <button onClick={() => setSelectedProduct(product)} className="px-4 py-2 rounded bg-gray-100 text-sm">
                              Edit
                            </button>
                            <button onClick={() => handleDelete(product.id)} className="px-4 py-2 rounded bg-red-50 text-red-600 text-sm">
                              Delete
                            </button>
                          </div>
                        </div>
