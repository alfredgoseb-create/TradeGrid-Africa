...
export default function Dashboard() {
  ...
  async function handleUpdate(product) {
    const { error } = await supabase.from("products").update({ name: product.name, category: product.category, description: product.description, unit: product.unit, status: product.status, supplier: product.supplier, stock_level: Number(product.stock_level) || 0, image_url: product.image_url }).eq("id", product.id);
            ...
            <div className="space-y-4">
              {products.map((product) => (
                <div key={product.id} className="border rounded-xl p-4 flex gap-4">
                  {product.image_url && <img src={product.image_url} alt={product.name} className="h-20 w-20 object-cover rounded-lg flex-shrink-0" />}
