...
export default function Dashboard() {
  ...
  async function handleUpdate(product: any) {
    const { error } = await supabase.from("products")
      .update({ price: product.price, stock: product.stock, description: product.description })
      .eq("id", product.id);
  ...
  const avgPrice = products.length
    ? (products.reduce((acc, item) => acc + Number(item.price || 0), 0) / products.length).toFixed(2)
    : "0.00";
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-10">
          ...
          <div className="bg-white rounded-xl shadow p-6">
            ...
            <p className="text-gray-500">Total stock: {totalStock}</p>
            <p className="text-gray-500">Average price: N${avgPrice}</p>
          </div>
        </div>
                  ...
                  <div className="flex-1">
                    {selectedProduct && selectedProduct.id === product.id ? (
                      <div className="space-y-2">
                        <input
                          type="text" value={selectedProduct.name}
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          />
                          <input
                            type="number" value={selectedProduct.stock}
